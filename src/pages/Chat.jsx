...
export default function ChatUser() {
    ...
    function lastMessage(data){
        const dataResult = data.map(d=>{
            ...
            const idLastMsgRecipient = d.recipientMessage[d.recipientMessage.length-1].id
        ...
        <div className="container">
           <div className="row p-3 mx-5 container-chat">
                <Col  lg={4} md={3} sm={3}>
                    ...
                    <div className="row">
                        <Contact fullName={dataUser.fullName} msg={"Online"} user={true} image={dataUser.image}/>
                    </div>
                                    ...
                                    <>
                                        {
                                            listDataContacts?.map((data, i)=>{
                                                let active =true
                                                return(
                                                    <Contact active={active} msg={data.msg} key={i} cotactId={contact?.id} idUser={data.id} fullName={data.fullName} image={data.image} handleActiveContact={handleActiveContact} />
                                                )
                        ...
                        contact?(
                            <>
                                                            ...
                                                            <>
                                                                {
                                                                    messages.map((m, i)=>{
                                                                        ...
                                                                        return(
                                                                            <Message key={i} image={imageIcon} receiver={receiver} contact={contact?.id} msg={m.message}  />
                                                                        )
                                ...
                                <div className="row mt-3 container-send-message">
                                    <Col lg={11} className="">
                                        <input ref={refInp} placeholder="Type a message" className="inp-send-message"/>
                                    </Col>
